...
| `batches` → `batch.patch` | `batches` → `batch.patch` | ✅ the frame carries a whole `BatchView`, so a client that reconnects mid-run learns the shape of one from the next member that moves. `stores/batches.js` merges it under the node whose socket delivered it; the ops tray groups by `runId` |
