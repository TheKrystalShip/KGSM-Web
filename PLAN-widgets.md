...
| **P3** ◑ | `useKeyedResource` and `leafLogsStore` are **done** — two leaf journals coexist, proven in a browser by `dash-pinning.mjs`. Still owed: `logsStore`, `logSourcesStore`, `servicesStore`. | **The leaf-logs case works.** Host logs and the services board still hold one target. |
